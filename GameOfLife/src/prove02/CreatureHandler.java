	...
	public void updateCreatures() {
		...
		for(Creature c : _creatures) {
				...
				if (c instanceof Spawner)
				{
					...
					if (baby != null)
					{
                        Point cubLocation = (Point)c.getLocation().clone();
                        cubLocation.x--;
                        baby.setLocation(cubLocation);
                        try
                        {
                            tempCreatures.add(baby);
                        }
                        catch (Exception e)
                        {
                            e.getMessage();
				...
				if (c instanceof Morphable)
				{
					Morphable m = (Morphable) c;
					Creature changed = m.changeType();
					if (changed != null)
					{
                        Point changedLocation = (Point)c.getLocation().clone();
                        changedLocation.y++;
                        changed.setLocation(changedLocation);
                        try
                        {
                            tempCreatures.add(changed);
                        }
                        catch (Exception e)
                        {
                            e.getMessage();
			...
			try
            {
                _creatures.addAll(tempCreatures);
            } catch (Exception e)
            {
                e.getMessage();
            }
